...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    List<Post> postFindByInput(String input);

    @Query("select distinct p from Post p join fetch p.member m where m.name = :name ")
    List<Post> postFindByName(String name);

    @Query("select distinct p from Post p join fetch p.member m where m.part = :part")
    List<Post> postFindByPart(Part part);
}
